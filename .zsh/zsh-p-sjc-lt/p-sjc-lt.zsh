...
DESCRIPTION
  Upload data and fetch URL from the pastebin http://p.sjc.lt
	...
	if [ -t 0 ]; then
	  ...
	  if [ "$*" ]; then
		...
		if [ -f "$*" ]; then
		  ...
		  echo "$*"
		fi | curl -d name=$user --data-urlencode text@$* http://p.sjc.lt/index.php/api/create
	  else
	  ...
	  while read -r line ; do
        curl -d $user -d name=$user --data-urlencode text=$line http://p.sjc.lt/index.php/api/create
	  done
	fi
}
